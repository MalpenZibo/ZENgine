...
use zengine::{
    ecs::{
        ...
        system::IntoSystem,
        system_parameter::{OptionalRes, Res},
    },
    event::{input_system, Bindings, InputHandler},
    math::{Transform, Vector2},
    Component, Engine, InputType, Resource, SpriteType,
};
...
fn ai_pad_control(
    ...
    mut ball_query: Query<(&Ball, &Transform)>,
) {
    ...
    {
        for (_, pad, transform) in ai_query.iter() {
