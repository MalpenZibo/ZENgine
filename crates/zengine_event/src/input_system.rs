use zengine_ecs::system_parameter::{Local, Res, ResMut};

use crate::{input::InputEvent, Bindings, EventStream, InputHandler, InputType, SubscriptionToken};

pub fn input_system<T: InputType>(
    bindings: Bindings<T>,
) -> impl Fn(Res<EventStream<InputEvent>>, ResMut<InputHandler<T>>, Local<Option<SubscriptionToken>>)
{
    move |event_stream: Res<EventStream<InputEvent>>,
          mut input_handler: ResMut<InputHandler<T>>,
          input_stream_token: Local<Option<SubscriptionToken>>| {
        if let Some(token) = input_stream_token {
...
mod tests {
    ...
    fn setup_test() -> (World, impl System) {
        ...
        world.create_resource(EventStream::<InputEvent>::default());
    ...
    fn retrieve_action_value() {
        ...
        {
            let mut input_stream = world.get_mut_resource::<EventStream<InputEvent>>().unwrap();
