...
fn text_render() -> impl FnMut(
    Option<ResMut<TextRenderer>>,
    Option<ResMut<TextAtlas>>,
    Query<(&Text, &Transform)>,
    Option<Res<Assets<Font>>>,
    Option<Res<Device>>,
    Option<Res<Queue>>,
    ResMut<RenderContextInstance>,
    Option<ResMut<CameraBuffer>>,
    Res<WindowSpecs>,
    Res<UsedCamera>,
) {
    let mut layouts: Vec<Layout<GlyphUserData>> = Vec::default();
    ...
    move |text_renderer,
        ...
        if let (
            ...
            if !fonts.is_empty() {
                let scale =
                    window_specs.size.as_vec2() / used_camera.get_size().unwrap_or(Vec2::ONE);
